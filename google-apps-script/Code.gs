...
function doPost(e) {
  try {
    ...
    if (!sheet) {
      ...
      sheet.appendRow([
        'Timestamp','Student Full Name','Email','Age','Date of Birth','Parent/Guardian Name',
        'Phone / WhatsApp Number','City, State, Country','Learned Carnatic Music Before?',
        'Learned Veena Before?','Veena Learning Duration','Currently Own a Veena?',
        'How Did You Hear About Classes?','Referral Student Name','Aspiration in Veena',
        'Five Songs to Play in Future','Source'
      ]);
      sheet.setFrozenRows(1);
    }

    sheet.appendRow([
      new Date(), data.studentName || '', data.email || '', data.age || '', data.dateOfBirth || '',
      data.parentGuardianName || '', data.phone || '', data.cityStateCountry || '',
      data.learnedCarnatic || '', data.learnedVeena || '', data.veenaLearningDuration || '',
      data.ownVeena || '', data.heardAbout || '', data.referralName || '', data.aspiration || '',
      data.futureSongs || '', data.source || 'Website'
    ]);

    if (data.email) {
      const subject = 'Welcome to Veeniksha — registration received';
      const html = `<div style="font-family:Arial,sans-serif;color:#172033;max-width:620px;margin:auto"><div style="background:#071b3a;color:white;padding:28px;border-radius:18px 18px 0 0"><h1 style="margin:0;font-family:Georgia,serif">Veeniksha Veena & Music Academy</h1><p style="color:#e7b85d;margin:6px 0 0">Traditional music. Personal guidance. Global learning.</p></div><div style="padding:28px;border:1px solid #eadfce;border-top:0"><p>Dear ${escapeHtml_(data.studentName || 'Student')},</p><p>Thank you for registering with Veeniksha. We have received your details and will contact you shortly with the next steps.</p><p>Warm regards,<br><b>Mrs. Monisha</b><br>Veeniksha Veena & Music Academy</p></div></div>`;
